...
export default {
	...
	mounted() {
		let pos = this.pos;
		let elem = document.getElementsByClassName("static-dropdown")[0];
		let rect = elem.getBoundingClientRect();
		this.setPosition(pos, elem, rect);
		// check if the "static-dropdown" is in viewport
	...
	methods: {
		...
		onResize(e) {
			if (document.getElementsByClassName("static-dropdown")[0]) {
				let elem = document.getElementsByClassName("static-dropdown")[0];
				let rect = elem.getBoundingClientRect();
				let triggeringElem = elem.closest(".popover");
				let rectTriggeringElem = triggeringElem.getBoundingClientRect();
		...
		setPopoverPosition(elem, rect, rectTriggeringElem) {
			if (
				rect.bottom > window.innerHeight ||
				window.innerHeight < rectTriggeringElem.bottom + rect.height
			) {
			...
			if (
				rect.right > window.innerWidth ||
				window.innerWidth < rectTriggeringElem.right + rect.width
			) {
