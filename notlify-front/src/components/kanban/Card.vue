<template>
	<div class="kb__column--cards">
		...
		<div class="kb-card__container" v-for="card in cards" :key="card.id" :data-id="card.id">
			<div class="kb__card" draggable="true" @dragstart="onDragStart($event, card.id)" @drag="onDrag" @dragenter.prevent @dragover.prevent @dblclick="onDblClick($event, card.id)" style="position: relative;">
