	...
	<div class="workspace">
		...
		>
			...
			<div
				...
				:key="`${list.key}+${card.id}`"
				class="drag-el"
				draggable="true"
				@dragstart="startDrag($event, card, list.key)"
			>
				{{ card.title }}
			</div>
			<div class="add-card__container">
				<div v-if="list.key !== listKey">
					<span @click="cardHandler('add', list.key)" class="add-card"
						>Add a card</span
					>
				</div>
				<div v-if="list.key === listKey && isAddingCard">
					<input
						type="text"
						v-model="list.newCard"
						:ref="`cardtitle-${list.key}`"
						@keypress.enter="cardHandler('save', list.key)"
						placeholder="Enter a card title"
					/>
					<div
						@click="cardHandler('save', list.key)"
						class="save-card"
					>
						Save
					</div>
			...
			lists: [
				...
				},
				{
					...
					cards: [
						{
							id: 0,
							title: "Drag notes within list to rearrange.",
						},
